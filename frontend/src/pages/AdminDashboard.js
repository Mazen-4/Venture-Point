  ...
  return (
    <>
      ...
      >
        ...
        {/* ...existing code... */}
        {/* Sidebar for desktop */}
          <aside className="hidden md:flex flex-col min-w-72 w-72 max-w-72 bg-white/90 backdrop-blur-lg shadow-2xl p-8 rounded-3xl animate-slidein-left transition-all duration-700 border-2 border-blue-200 fixed h-[calc(97vh)] z-30 landscape:static landscape:relative top-6 left-2 md:top-8 md:left-3">
            <div className="mb-6">
              <h2 className="text-3xl font-bold text-center text-blue-700">Admin Panel</h2>
            </div>
              ...
              <ul className="space-y-3">
                {navItems.map((item, idx) => (
                  <li key={item.name} className="animate-fadein" style={{ animationDelay: `${idx * 0.07}s` }}>
                    <Link
                      to={item.path}
                      className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-green-50 text-green-900 font-semibold transition-all duration-300 golden-frame"
                    >
                      {item.icon}
                      {item.name}
                    </Link>
          ...
          <div className="mt-auto pt-6 border-t">
            <button
              ...
              className="w-full px-4 py-3 bg-gradient-to-r from-red-500 to-red-700 text-white rounded-xl hover:scale-105 shadow-lg transition-all duration-300 border-2 border-red-300 logout-button"
            >
              Logout
            </button>
          </div>
                ...
                <ul className="space-y-3">
                  {navItems.map((item) => (
                    <li key={item.name}>
                      <Link
                        to={item.path}
                        className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-green-800 text-white font-semibold transition-all duration-300 golden-frame"
                        onClick={() => setSidebarOpen(false)}
                      >
                        {item.icon}
                        {item.name}
