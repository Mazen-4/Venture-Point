...
export default function AdminAdvisors() {
  ...
  const handleEditClick = (advisor) => {
    setEditForm({ id: advisor.id, name: advisor.name, area_of_focus: advisor.area_of_focus, bio: advisor.bio });
    setEditImage(null);
              ...
              {advisors.length === 0 ? (
                ...
                advisors.map((advisor) => (
                  <tr key={advisor.id} className="hover:bg-blue-50 transition-colors">
                    <td className="py-4 px-4 font-medium text-gray-900">{advisor.id}</td>
